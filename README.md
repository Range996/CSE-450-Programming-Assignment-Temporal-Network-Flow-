...
![images](images/ScratchNodes.png)
...
#### Step 1: data grabbing. Read the whole data from txt file, and insert into an object defined as ![images](images/singleFlightSchedule.png) . 
#### Step 2: data processing. The SingleFlightSchedule are stored in a list, named as flightSchedule.  ![images](images/flightSchedules.png)
Then, processing the SingleFlightSchedule data one by one, for example “LAX JFK	0 9 185”  is stored into a list of map named listOfAirports as a map of map data structure. ![images](images/listOfAirports.png)
...
3) Return flow.
After building the graph, applied the FordFulkson algorithm to the graph, before we do so, we shall transfer the list of airports to a two dimension array.![images](images/graphArray.png)  , numpoinmts means the number of the total nodes.
And the final result is 6751. 
![images](images/result.png)

Insturciton:
	Using IntelliJ to open the project, and the JDK version is 8, ![images](images/jdkVersion.png) . And then build and run the Main.java.
